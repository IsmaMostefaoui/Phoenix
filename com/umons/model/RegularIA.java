...
public class RegularIA extends Player implements IRobot{
	...
	public Location chooseWall(Player[] players, IPathFinder finder) {
		...
		Path[] pathtab = testFinderMove(players, finder);
		Path opponent = pathtab[1];
		Location nextWall = null;
		for (Location wall : allWall) {
			for (int i = 0; i < ((Grid.getLen()/2)+1); i++) {
				Path path = finder.findPath(wall, players[1].getLoc().getLocX(), players[1].getLoc().getLocY(), 2*i, players[1].getCoordFinish());
				if ((path != null && path.getLength() > opponent.getLength())) {
					opponent = finder.findPath(wall, players[1].getLoc().getLocX(), players[1].getLoc().getLocY(), 2*i, players[1].getCoordFinish());
					System.out.println("Opponent lenght = " + opponent.getLength());
					nextWall = wall;
