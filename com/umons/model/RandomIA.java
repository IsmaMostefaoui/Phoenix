...
public class RandomIA extends Player {
	...
	public RandomIA(Location loc, int NbreOfWall ,int OrderNumber) {
		super(loc, NbreOfWall, OrderNumber);	
	}
	 ...
	 */
	public void play(Grid grid, Player opponent) {
	...
	public boolean putwall(Grid grid, Player opponent) {
		...
		int numb = 0;
		while (numb < 4) {
			if (super.putWall(grid, tabWall[numb])) {
				return true;
			}
			numb++;
		}
